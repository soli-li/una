...
public interface MenuDao extends BaseRepository<Menu, String> {
  @Query(nativeQuery = true,
      value = """
          SELECT M.ID, M.NAME, M.ICON, M.SORT, M.FRONT_END_URI, M.PARENT_ID, M.PERMISSIONS_ID,
                 M.STATUS, M.REMARK, M.CREATED_USER_ID, M.CREATED_DATE
          FROM S_MENU M
  ...
  @Query(nativeQuery = false,
      value = """
          ...
          WHERE 1 = 1
                ...
                AND (:#{#sm.eqFrontEndUri} IS NULL
                    OR UPPER(m.frontEndUri) = UPPER(:#{#sm.eqFrontEndUri}))
          """)
