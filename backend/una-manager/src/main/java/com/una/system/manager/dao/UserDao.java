...
public interface UserDao extends BaseRepository<User, String> {
  ...
  @Query(nativeQuery = true,
      value = """
          SELECT U.ID, U.COMPANY_ID, U.USERNAME, U.PASSWORD, U.ACCOUNT_NON_EXPIRED,
                 U.ACCOUNT_NON_LOCKED, U.CREDENTIALS_NON_EXPIRED, U.DEFAULT_GROUP_ID,
                 U.LAST_CHANGE_PW_DATE, U.LAST_LOGIN_DATE, U.FAILURE_COUNT, U.CREATED_USER_ID,
                 U.CREATED_DATE, U.UPDATED_USER_ID, U.UPDATED_DATE
  ...
  @Query(nativeQuery = true,
      value = """
          ...
          VALUES(?1, ?2, ?3, ?4, ?5);
          """)
  void saveGroupRelation(String id, String userId, String groupId, String createdUserId,
  ...
  @Query(nativeQuery = true,
      value = """
          ...
          VALUES(?1, ?2, ?3, ?4, ?5);
            """)
  void saveRoleRelation(String id, String userId, String roleId, String createdUserId,
      LocalDateTime dateTime);

  @Modifying
  @Query(nativeQuery = true,
      value = """
          INSERT INTO S_USER(ID, COMPANY_ID, USERNAME, PASSWORD, ACCOUNT_NON_EXPIRED,
                            ACCOUNT_NON_LOCKED, CREDENTIALS_NON_EXPIRED, DEFAULT_GROUP_ID,
                            LAST_LOGIN_DATE, LAST_CHANGE_PW_DATE, FAILURE_COUNT, CREATED_USER_ID,
                            CREATED_DATE, UPDATED_USER_ID, UPDATED_DATE)
          VALUES(:#{#u.id}, :#{#u.companyId}, :#{#u.username}, '',
                 :#{#u.accountNonExpired ? 'Y' : 'N'}, :#{#u.accountNonLocked ? 'Y' : 'N'},
                 :#{#u.credentialsNonExpired ? 'Y' : 'N'}, :#{#u.defaultGroupId}, NULL, NULL,
                 :#{#u.failureCount}, :#{#u.createdUserId}, :#{#u.createdDate},
                 :#{#u.updatedUserId}, :#{#u.updatedDate})
          """)
  ...
  @Query(nativeQuery = false,
      value = """
          ...
          """)
  Page<Object[]> search(@Param("su") SearchUser searchUser, @Param("id") String companyId,
