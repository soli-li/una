...
public class ResponseHeaderInterceptor implements ResponseBodyAdvice<Object> {
  ...
  public Object beforeBodyWrite(final Object body, final MethodParameter returnType,
    ...
    final String mimeType = String.format("%s/%s", selectedContentType.getType(),
        selectedContentType.getSubtype());
    final Charset charset = selectedContentType.getCharset();
    if (StringUtils.equalsAny(mimeType, MimeTypeUtils.TEXT_PLAIN_VALUE,
        MimeTypeUtils.APPLICATION_JSON_VALUE) && Objects.isNull(charset)) {

      final MediaType mediaType = MediaType
          .parseMediaType(String.format("%s;charset=%s", mimeType, this.encoding));
