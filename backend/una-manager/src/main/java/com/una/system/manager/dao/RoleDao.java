...
public interface RoleDao extends BaseRepository<Role, String> {
  ...
  @Query(nativeQuery = true,
      value = """
          SELECT R.ID, R.COMPANY_ID, R.AUTHORITY, R.STATUS, R.REMARK, R.CREATED_USER_ID,
                 R.CREATED_DATE, R.UPDATED_USER_ID, R.UPDATED_DATE
  ...
  @Query(nativeQuery = true,
      value = """
          ...
          VALUES(?1, ?2, ?3, ?4, ?5);
            """)
  void saveGroupRelation(String id, String groupId, String roleId, String createdUserId,
  ...
  @Query(nativeQuery = true,
      value = """
          ...
          VALUES(?1, ?2, ?3, ?4, ?5);
            """)
  void savePermRelation(String id, String groupId, String roleId, String createdUserId,
      LocalDateTime dateTime);
  ...
  @Query(nativeQuery = true,
      value = """
          ...
          VALUES(?1, ?2, ?3, ?4, ?5);
          """)
  void saveUserRelation(String id, String userId, String roleId, String createdUserId,
      LocalDateTime dateTime);
  ...
  @Query(nativeQuery = false,
      value = """
          ...
          WHERE (:allow = TRUE OR r.companyId = :id)
                ...
                AND (:#{#sr.eqAuthority} IS NULL
                    OR (UPPER(r.authority) = UPPER(:#{#sr.eqAuthority})
                       AND :#{#sr.companyId} = r.companyId))
          """)
  Page<Object[]> search(@Param("sr") SearchRole searchRole, @Param("id") String companyId,
