...
public class ConfigurationService {
  ...
  public void refresh() {
    ...
    final List<Configuration> configList = Optional.ofNullable(paginationInfo.getDataList())
        .orElse(List.of());
    ...
    if (StringUtils.equals("normal", this.runningMode)) {
      final List<String> allConfigurationList = Optional
          .ofNullable(this.sharingService.getAllConfiguration()).orElse(List.of());
  ...
  public void save(final Configuration configuration, final String userId) {
    ...
    Assert.isTrue(
        StringUtils.equalsAny(configuration.getStatus(), Constants.ENABLE, Constants.DISABLE),
    ...
    if (StringUtils.isBlank(configuration.getId())) {
      ...
      final Optional<Configuration> optional = this.configurationDao
    ...
    ConfigurationService.LOGGER.info(
        "save or update configuration object -> already checked configuration value, ready save");
  ...
  @Transactional(readOnly = true,
      propagation = Propagation.SUPPORTS)
  public PaginationInfo<Configuration> search(final SearchConf searchConf, final String companyId,
    ...
    searchConf.setName(
        StringUtils.wrapIfMissing(StringUtils.defaultIfBlank(searchConf.getName(), null), "%"));
    searchConf.setConfKey(
        StringUtils.wrapIfMissing(StringUtils.defaultIfBlank(searchConf.getConfKey(), null), "%"));
    searchConf.setCompanyName(StringUtils
        .wrapIfMissing(StringUtils.defaultIfBlank(searchConf.getCompanyName(), null), "%"));

    Pageable pageable = Pageable.unpaged();
    if (searchConf.isPageable()) {
      final List<Order> orderList = PaginationSortUtil.transformToOrder(searchConf.getSort());
      pageable = PageRequest.of(searchConf.getPageNumber() - 1, searchConf.getPageSize(),
          Sort.by(orderList));
    }

    ConfigurationService.LOGGER.info("search user, current{} contain pagination information",
        pageable.isPaged() ? "" : " does not");
    ConfigurationService.LOGGER.info("search condition, {}", searchConf.toString());

    final Page<Configuration> result = this.configurationDao
        .search(searchConf, companyId, allowNotCompanyCond, pageable).map(o -> {
          final User createdUser = new User();
          final Transform3<Configuration, Company, User> transform = ObjectsToEntityUtil
              .getTransform3(o);
    ...
    final PaginationInfo<Configuration> paginationInfo = PaginationInfo.of(
        searchConf.getPageNumber(), searchConf.getPageSize(), searchConf.isPageable(),
  ...
  private int sort(final Configuration o1, final Configuration o2) {
    return StringUtils.defaultIfBlank(o1.getName(), "")
        .compareTo(StringUtils.defaultString(o2.getName(), ""));
  }
  ...
  public void syncFormManage(final String userId) {
    ...
    final List<Configuration> allConfList = Optional
        .ofNullable(this.search(searchConf, user.getCompanyId(), true).getDataList())
        .orElse(List.of());
    final Map<String, List<Configuration>> configMap = new HashMap<>();
    for (final Configuration configuration : allConfList) {
      final List<Configuration> list = configMap.getOrDefault(configuration.getCompanyId(),
          new ArrayList<>());
    ...
    final List<Company> companyList = Optional
        .ofNullable(
            this.companyService.search(searchCompany, user.getCompanyId(), true).getDataList())
        .orElse(List.of());
    ...
    final Map<String, Configuration> manageConfigMap = Optional
        .ofNullable(configMap.remove(user.getCompanyId())).orElse(List.of()).stream()
    ...
    for (final Map.Entry<String, List<Configuration>> entry : configMap.entrySet()) {
      final Map<String, Configuration> companyConfigMap = Optional.ofNullable(entry.getValue())
          .orElse(List.of()).stream().collect(Collectors.toMap(Configuration::getConfKey, c -> c));
