...
public interface GroupDao extends BaseRepository<Group, String> {
  ...
  @Query(nativeQuery = true,
      value = """
          SELECT G.ID, G.COMPANY_ID, G.PARENT_ID, G.NAME, G.STATUS, G.REMARK, G.CREATED_USER_ID,
                 G.CREATED_DATE, G.UPDATED_USER_ID, G.UPDATED_DATE
  ...
  @Query(nativeQuery = true,
      value = """
          ...
          VALUES(?1, ?2, ?3, ?4, ?5);
            """)
  void saveRoleRelation(String id, String groupId, String roleId, String createdUserId,
  ...
  @Query(nativeQuery = true,
      value = """
          ...
          VALUES(?1, ?2, ?3, ?4, ?5);
          """)
  void saveUserRelation(String id, String userId, String groupId, String createdUserId,
      LocalDateTime dateTime);
  ...
  @Query(nativeQuery = false,
      value = """
          ...
          WHERE (:allow = TRUE OR g.companyId = :id)
                ...
                AND (:#{#sg.eqName} IS NULL
                    OR (UPPER(g.name) = UPPER(:#{#sg.eqName}) AND :#{#sg.companyId} = g.companyId))
          """)
  Page<Object[]> search(@Param("sg") SearchGroup searchGroup, @Param("id") String companyId,
