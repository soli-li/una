...
public class UserHandler {
  ...
  public Mono<ServerResponse> addUser(final ServerRequest request) {
    ...
    final Mono<Optional<User>> requestMono = request.bodyToMono(User.class).flatMap(u -> Mono.just(Optional.ofNullable(u))).defaultIfEmpty(Optional.empty());
    final Mono<Optional<User>> addUserMono = this.addUser(hasPreUuid, requestMono).flatMap(o -> Mono.just(Optional.of(o))).defaultIfEmpty(Optional.empty());
    final Mono<Boolean> result = this.addUserManage(addUserMono, uuid, String.join(",", headers.header(com.una.common.Constants.REMOTE_ADDRESS)),
  ...
  private Mono<User> addUser(final String uuid, final Mono<Optional<User>> userMono) {
    return userMono.flatMap(o -> {
      ...
      ResultMonoUtil.assertTrue(Objects.isNull(user) || StringUtils.isAnyBlank(user.getUsername(), user.getId()), "user is null", UserHandler.LOGGER);

      final Mono<Boolean> result = this.userRedisOperations.opsForValue().set(uuid, user, Duration.of(this.userTimeout, ChronoUnit.SECONDS));
      return result.map(b -> {
        ResultMonoUtil.assertTrue(!b, "cannot add user to redis, username: " + user.getUsername(), UserHandler.LOGGER);
        return user;
      });
    });
  }

  private Mono<Boolean> addUserManage(final Mono<Optional<User>> userMono, final String sessioinId, final String ip, final String userAgent) {
    final Map<String, String> contextMap = MDC.getCopyOfContextMap();
    final Mono<User> mono = userMono.map(o -> {
      MDC.setContextMap(contextMap);
      final String username = o.map(User::getUsername).orElse("");
      ResultMonoUtil.assertTrue(o.isEmpty() || StringUtils.isAnyBlank(sessioinId, username), "cannot add user session to redis, username: " + username,
        UserHandler.LOGGER);
      return o.get();
    });
    ...
    final Mono<Boolean> result = mono.flatMap(u -> {
      ...
      final Mono<Optional<UserSessionList>> userSessionMono = ResultMonoUtil.getExistOrEmptyValue(this.manangerUserRedisOperations, managerUuid, null, true)
      ...
      return saveResult.map(b -> {
        MDC.setContextMap(contextMap);
        ResultMonoUtil.assertTrue(!b, "cannot add user session to redis, username: " + u.getUsername(), UserHandler.LOGGER);
        return b;
      });
    });
  ...
  public Mono<ServerResponse> getUserManage(final ServerRequest request) {
    ...
    ResultMonoUtil.assertTrue(StringUtils.isBlank(username), "username is null", UserHandler.LOGGER);

    final String userManagerSessionKey = this.getUserManagerSessionKey(username);
    final Mono<UserSessionList> responseMono = ResultMonoUtil.getExistOrEmptyValue(this.manangerUserRedisOperations, userManagerSessionKey, null,
      ignoreNotExist);
  ...
  public Mono<ServerResponse> getUserOrEmpty(final ServerRequest request) {
    ...
    final Mono<User> responseMono = ResultMonoUtil.getExistOrEmptyValue(this.userRedisOperations, Constants.REDIS_USER_PRE + id,
      Duration.of(this.userTimeout, ChronoUnit.SECONDS), ignoreNotExist);
    return ResultMonoUtil.getSuccessResponse(vo, responseMono);
  }
  ...
  public Mono<ServerResponse> removeUserManage(final ServerRequest request) {
    ...
    final String username = request.pathVariable("username");
    ResultMonoUtil.assertTrue(StringUtils.isBlank(username), "username is empty", UserHandler.LOGGER);

    final Mono<Long> responseMono = this.manangerUserRedisOperations.delete(this.getUserManagerSessionKey(username));
    return ResultMonoUtil.getSuccessResponse(vo, responseMono);
  }
  ...
  public Mono<ServerResponse> updateUser(final ServerRequest request) {
    ...
    ResultMonoUtil.assertTrue(StringUtils.isBlank(sessionId), "request parameter exception", UserHandler.LOGGER);
    ...
    final Mono<Optional<User>> requestMono = request.bodyToMono(forType).flatMap(u -> Mono.just(Optional.ofNullable(u))).defaultIfEmpty(Optional.empty());
    final Mono<Boolean> responseMono = requestMono.flatMap(o -> {
      MDC.setContextMap(contextMap);

      final User user = o.orElse(null);
      ResultMonoUtil.assertTrue(Objects.isNull(user), "request parameter exception", UserHandler.LOGGER);
  ...
  public Mono<ServerResponse> updateUserManage(final ServerRequest request) {
    ...
    final ParameterizedTypeReference<UserSessionList> refernece = new ParameterizedTypeReference<>() {
    };
    final Mono<Optional<UserSessionList>> requestMono = request.bodyToMono(refernece).flatMap(m -> Mono.just(Optional.ofNullable(m)))
    ...
    final Mono<Boolean> responseMono = requestMono.flatMap(o -> {
      ...
      return this.manangerUserRedisOperations.hasKey(userSessionList.getKey()).flatMap(exist -> {
        MDC.setContextMap(contextMap);
        if (exist) {
          return this.manangerUserRedisOperations.opsForValue().set(userSessionList.getKey(), userSessionList);
        }
        else {
