...
@SpringBootTest(classes = SharingApplication.class, webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
@Rollback(value = true)
...
public class SharingApplicationTests {
  ...
  private void clearConfRedis() {
    final ScanOptionsBuilder scanOptionsBuilder = ScanOptions.scanOptions().match(Constants.REDIS_CONF_PRE + "*").count(10);
  ...
  private void clearManangerUserRedis() {
    final ScanOptionsBuilder scanOptionsBuilder = ScanOptions.scanOptions().match(Constants.REDIS_MANAGER_USER_PRE + "*").count(10);
  ...
  private void clearShortTerm() {
    final ScanOptionsBuilder scanOptionsBuilder = ScanOptions.scanOptions().match(Constants.REDIS_SHORT_TERM_PRE + "*").count(10);
  ...
  private void clearUserRedis() {
    final ScanOptionsBuilder scanOptionsBuilder = ScanOptions.scanOptions().match(Constants.REDIS_USER_PRE + "*").count(10);
