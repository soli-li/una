...
public class NormalUriInterceptor implements HandlerInterceptor {
  ...
  public void init() {
    ...
    NormalUriInterceptor.LOGGER.info("using spring properties key '{}', value is: {}, default value is: {}", NormalUriInterceptor.SPRING_REQUEST_OUT_UUID,
      value, defaultValue);
    NormalUriInterceptor.outUuid = Boolean.parseBoolean(environment.getProperty(NormalUriInterceptor.SPRING_REQUEST_OUT_UUID, defaultValue));

    value = environment.getProperty(NormalUriInterceptor.SPRING_USING_HEAD_UUID);
    NormalUriInterceptor.LOGGER.info("using spring properties key '{}', value is: {}", NormalUriInterceptor.SPRING_USING_HEAD_UUID, value);
    NormalUriInterceptor.headUuidKey = environment.getProperty(NormalUriInterceptor.SPRING_USING_HEAD_UUID);
  }
  ...
  public void markToMdc(final ServletRequest request) {
    ...
    if (request instanceof final HttpServletRequest httpRequest && StringUtils.hasText(NormalUriInterceptor.headUuidKey)) {
      uuid = Optional.ofNullable(httpRequest.getHeader(NormalUriInterceptor.headUuidKey)).orElse("");
    }
    else {
  ...
  public void postHandle(final HttpServletRequest request, final HttpServletResponse response, final Object handler, final ModelAndView modelAndView)
    ...
    if (Objects.isNull(endTimeFlag)) {
      final LocalDateTime localDateTime = (LocalDateTime) request.getAttribute(NormalUriInterceptor.START_TIME);
      ...
      NormalUriInterceptor.LOGGER.info("requested done, request uri: {}, now is: {}, used {}ms", request.getRequestURI(),
  ...
  public boolean preHandle(final HttpServletRequest request, final HttpServletResponse response, final Object handler) throws Exception {
    ...
    if (Objects.isNull(startTimeFlag)) {
      ...
      NormalUriInterceptor.LOGGER.info("request uri: {}, now is: {}", request.getRequestURI(),
