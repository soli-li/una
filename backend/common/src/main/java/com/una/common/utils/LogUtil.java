...
public final class LogUtil {
  ...
  static {
    ...
    final String info = String.format("using system properties key '%s', value is %s, default value is: %s", logMarkKey, markValue, defaultLogMark);
  ...
  private static StackTraceElement getOriginClass() {
    ...
    for (final StackTraceElement element : stackTraceElements) {
      ...
      if (!Thread.class.getName().equals(originClass) && originClass.indexOf(LogUtil.class.getName()) < 0) {
        return element;
      }
    }
    return null;
