...
public class UrlPermService {
  ...
  public void save(final UrlPerm urlPerm, final String userId) {
    ...
    Assert.isTrue(StringUtils.equalsAny(urlPerm.getStatus(), Constants.ENABLE, Constants.DISABLE), "incorrect status value");
    ...
    UrlPermService.LOGGER.info("save or update url perm object -> already checked url perm value, ready save");
  ...
  public PaginationInfo<UrlPerm> search(final SearchUrlPerm searchUrlPerm) {
    ...
    searchUrlPerm.setName(StringUtils.wrapIfMissing(StringUtils.defaultIfBlank(searchUrlPerm.getName(), null), "%"));
    searchUrlPerm.setUri(StringUtils.wrapIfMissing(StringUtils.defaultIfBlank(searchUrlPerm.getUri(), null), "%"));
    searchUrlPerm.setPerm(StringUtils.wrapIfMissing(StringUtils.defaultIfBlank(searchUrlPerm.getPerm(), null), "%"));

    Pageable pageable = Pageable.unpaged();
    if (searchUrlPerm.isPageable()) {
      final List<Order> orderList = PaginationSortUtil.transformToOrder(searchUrlPerm.getSort());
      pageable = PageRequest.of(searchUrlPerm.getPageNumber() - 1, searchUrlPerm.getPageSize(), Sort.by(orderList));
    }

    UrlPermService.LOGGER.info("search url perm, current{} contain pagination information", pageable.isPaged() ? "" : " does not");
    ...
    final PaginationInfo<UrlPerm> paginationInfo = PaginationInfo.of(searchUrlPerm.getPageNumber(), searchUrlPerm.getPageSize(), searchUrlPerm.isPageable(),
