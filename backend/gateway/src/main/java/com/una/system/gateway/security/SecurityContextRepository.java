...
public class SecurityContextRepository implements ServerSecurityContextRepository {
  ...
  public SecurityContextRepository(final String authHeaderName, final String userIdHeaderName, final String companyIdHeaderName,
    final SharingService sharingService, final String formLoginUrl) {
  ...
  public Mono<SecurityContext> load(final ServerWebExchange exchange) {
    ...
    final Mono<Optional<User>> mono = this.sharingService.loadUser(sessionId).flatMap(u -> Mono.just(Optional.of(u))).defaultIfEmpty(Optional.empty());
    return mono.flatMap(optional -> {
      if (optional.isEmpty()) {
        throw new RuntimeException("cannot found user");
      }
      final User user = optional.get();
      final Set<String> currentRoleIdSet = Optional.ofNullable(user.getCurrentRoleId()).orElse(Set.of());
      ...
      final UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken = new UsernamePasswordAuthenticationToken(user.getUsername(), "",
        authoritySet);
  ...
  public Mono<Void> save(final ServerWebExchange exchange, final SecurityContext context) {
    if (context.getAuthentication() instanceof final UsernamePasswordAuthenticationToken authentication
      ...
      Mono<Optional<Map<String, String>>> sessionResult = this.sharingService
        .addUser(user, remoteAddress.getAddress().getHostAddress(), String.join(",", headers.get(HttpHeaders.USER_AGENT)))
      ...
      return sessionResult.flatMap(o -> {
        ...
        try {
          exchange.getResponse().getHeaders().add(this.authHeaderName, map.get(Constants.SESSION_KEY));
          return Mono.empty();
        }
