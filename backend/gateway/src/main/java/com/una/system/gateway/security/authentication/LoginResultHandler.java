...
public class LoginResultHandler implements ServerAuthenticationSuccessHandler, ServerAuthenticationFailureHandler {
  ...
  public LoginResultHandler(final String encoding, final ManagerService managerService, final SharingService sharingService, final String authHeaderName) {
  ...
  @Override
  public Mono<Void> onAuthenticationFailure(final WebFilterExchange webFilterExchange, final AuthenticationException exception) {
...
""";
    if (exception instanceof final com.una.system.gateway.security.authentication.exception.AuthenticationException authenticationException) {
      try {
        LoginResultHandler.LOGGER.error("login failure, username: {}", authenticationException.getUsername(), authenticationException);
        final User user = authenticationException.getUser();
        Mono<Object> mono = Mono.just(new Object());
        if (Objects.nonNull(user)) {
          this.managerService.updateUser(user).subscribe();
          if (!user.isCredentialsNonExpired()) {
            mono = this.sharingService.generateShortSession(List.of(Constants.TEMP_ACTION_EXPIRED, user.getUsername())).zipWhen(map -> {
        ...
        final String errorCode = authenticationException.getErrorCode();
        final String errorMessage = authenticationException.getMessage();
        final byte[] message = String.format(format, errorCode, errorMessage).getBytes(this.encoding);
...
""";
    ...
    if (authentication instanceof final UsernamePasswordAuthenticationToken token && token.getDetails() instanceof final AuthenticationDetail detail) {
