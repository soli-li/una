...
import com.una.system.gateway.security.authentication.exception.AuthenticationException;
...
public class LoginAuthentication implements ReactiveAuthenticationManager {
  ...
  public LoginAuthentication(final ManagerService managerService, final SharingService sharingService, final int defaultMaximumSessions,
    final boolean defaultMaxSessionsPreventsLogin, final String maximumSessionsKey, final String maxSessionsPreventsLoginKey) {
  ...
  public Mono<Authentication> authenticate(final Authentication authentication) {
    ...
    throw new AuthenticationException(authentication.getName(), Constants.LOGIN_FAIL_CODE_NOT_SUPPORT_TOKEN.toString(), "not support authentication token");
  }
  ...
  private void checkParameter(final String username, final String password, final String captch) {
    ...
    if (!list.isEmpty()) {
      ...
      throw new AuthenticationException(username, Constants.LOGIN_FAIL_CODE_PARAM.toString(), msg);
    }
  }
  ...
  private Mono<Boolean> checkPassword(final String username, final String rawPassword, final User user) {
    Mono<Optional<Boolean>> mono = this.managerService.passwordMatch(username, rawPassword).flatMap(b -> Mono.just(Optional.of(b)));
    mono = mono.defaultIfEmpty(Optional.empty());
    return mono.map(o -> {
      final boolean isMatch = o.orElse(false);
      if (!isMatch) {
        final AuthenticationException authEx = new AuthenticationException(username, Constants.LOGIN_FAIL_CODE_PASS_NOT_MATCH.toString(), "password not match");
  ...
  private void checkUserStatus(final User user, final String username) {
    ...
    if (!user.isCredentialsNonExpired()) {
      authenticationException = new AuthenticationException(username, Constants.LOGIN_FAIL_CODE_USER_EXPIRED.toString(), "user account expired");
    }
    if (!user.isAccountNonExpired()) {
      authenticationException = new AuthenticationException(username, Constants.LOGIN_FAIL_CODE_USER_STATUS.toString(), "user account disabled");
    }
    if (!user.isAccountNonLocked()) {
      authenticationException = new AuthenticationException(username, Constants.LOGIN_FAIL_CODE_USER_LOCKED.toString(), "user account locked");
    }
  ...
  private Mono<Optional<User>> fillUser(final Mono<Boolean> mono, final User user) {
    return mono.flatMap(v -> this.managerService.fillUser(user).flatMap(u -> Mono.just(Optional.of(u))).defaultIfEmpty(Optional.empty()));
  }

  private Mono<UsernamePasswordAuthenticationToken> generateToken(final Mono<Optional<User>> mono, final AuthenticationDetail authDetail) {
    ...
    return mono.flatMap(ou -> {
      ...
      final Mono<UsernamePasswordAuthenticationToken> successTokenMono = Mono.fromCallable(() -> {
        ...
        user.getCurrentRoleId().forEach(id -> grantedAuthoritySet.add(new UserGrantedAuthority(id)));
        authDetail.setUser(user);
        final UsernamePasswordAuthenticationToken auth = new UsernamePasswordAuthenticationToken(user.getUsername(), "", grantedAuthoritySet);
        auth.setDetails(authDetail);
        return auth;
      });

      final Mono<List<Configuration>> configurationListMono = this.sharingService.getConfigurationList(user.getCompanyId());
      return configurationListMono.flatMap(list -> {
        final String maxConfValue = list.stream().filter(c -> StringUtils.equals(c.getConfKey(), this.maximumSessionsKey))
          .filter(c -> StringUtils.equals(c.getStatus(), com.una.common.Constants.ENABLE)).findAny().map(Configuration::getConfValue).orElse("");
        final String preventsConfValue = list.stream().filter(c -> StringUtils.equals(c.getConfKey(), this.maxSessionsPreventsLoginKey))
          .filter(c -> StringUtils.equals(c.getStatus(), com.una.common.Constants.ENABLE)).findAny().map(Configuration::getConfValue).orElse("");
  ...
  private Mono<UsernamePasswordAuthenticationToken> loginOver(final String username, final int maximumSessions, final boolean maxSessionsPreventsLogin,
    final Mono<UsernamePasswordAuthenticationToken> successMono) {
    final Mono<Optional<UserSessionList>> onlineSessionListMono = this.managerService.getUserSession(username).flatMap(o -> Mono.just(Optional.ofNullable(o)))
      .defaultIfEmpty(Optional.empty());
    return onlineSessionListMono.flatMap(o -> {
      final Map<String, SessionInfo> sessionMap = o.map(UserSessionList::getSessionMap).orElse(null);
      if (Objects.isNull(sessionMap) || sessionMap.size() < maximumSessions) {
        return successMono;
      }

      if (maxSessionsPreventsLogin) {
        throw new AuthenticationException(username, Constants.LOGIN_FAIL_CODE_OVER_SESSION.toString(), "over max sessions prevents login");
      }
  ...
  private void throwAuthenticationException(final Throwable e, final String username) {
    if (e instanceof final AuthenticationException authenticationException) {
    }
    else {
      throw new AuthenticationException(username, Constants.LOGIN_FAIL_CODE_100.toString(), "login authenticate exception", e);
    }
  }
  ...
  private Mono<Authentication> usernamePasswordAuthenticate(final Authentication authentication) {
    ...
    Mono<Optional<User>> mono = this.managerService.findUser(authentication.getName()).flatMap(u -> Mono.just(Optional.of(u)));
    mono = mono.defaultIfEmpty(Optional.empty());
    mono = mono.doOnError(e -> this.throwAuthenticationException(e, authentication.getName()));
    return mono.flatMap(o -> {
      if (o.isEmpty()) {
        throw new AuthenticationException(username, Constants.LOGIN_FAIL_CODE_USER_NOT_FOUND.toString(), "user not found");
      }
