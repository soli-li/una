...
public class AccessAuthorization implements ReactiveAuthorizationManager<AuthorizationContext> {
  ...
  public AccessAuthorization(final String permIdHeaderName, final ManagerService managerService, final ObjectMapper objectMapper) {
  ...
  public Mono<AuthorizationDecision> check(final Mono<Authentication> authentication, final AuthorizationContext object) {
    ...
    return authentication.flatMap(auth -> {
      ...
      if (auth instanceof final UsernamePasswordAuthenticationToken token && token.getDetails() instanceof final AuthenticationDetail detail) {
        ...
        final Mono<Tuple2<Tuple2<Set<String>, List<UrlPerm>>, Boolean>> hasPermission = this.hasPermission(urlPerm, path.value());
        ...
        return hasPermission.map(o -> {
          ...
          if (!b) {
            ...
            builder = builder.headers(h -> h.set(this.permIdHeaderName, this.object2String(permIdsSet)));
  ...
  private Mono<Set<String>> getPermIds(final Set<String> roleIds) {
    Mono<Optional<List<Permissions>>> mono = this.managerService.findPermByRole(roleIds).flatMap(o -> Mono.just(Optional.ofNullable(o)));
  ...
  private Mono<Tuple2<Set<String>, List<UrlPerm>>> getUrlPerm(final Mono<Set<String>> permIdsMono) {
    return permIdsMono.zipWhen(permIdsSet -> {
      ...
      Mono<Optional<List<UrlPerm>>> mono = this.managerService.findUrlByPerm(permIdsSet).flatMap(o -> Mono.just(Optional.ofNullable(o)));
      mono = mono.defaultIfEmpty(Optional.of(List.of()));
  ...
  private Mono<Tuple2<Tuple2<Set<String>, List<UrlPerm>>, Boolean>> hasPermission(final Mono<Tuple2<Set<String>, List<UrlPerm>>> urlPermMono,
    final String path) {
    return urlPermMono.zipWhen(o -> {
      final List<UrlPerm> urlList = o.getT2();
      Collections.sort(urlList, (o1, o2) -> Optional.ofNullable(o1.getSort()).orElse(0).compareTo(Optional.ofNullable(o2.getSort()).orElse(0)));
