...
public class AuthenticationConverter implements ServerAuthenticationConverter {
  ...
  public AuthenticationConverter(final String formLoginUrl, final String usernameParameter, final String passwordParameter, final String captchaParameter,
    final String method, final String encoding) {
  ...
  public Mono<Authentication> convert(final ServerWebExchange exchange) {
    ...
    xWwwFormUrlencodedList.add(StringUtils.startsWith(type, com.una.common.Constants.MEDIA_TYPE_X_WWW_FORM_URLENCODED.toString()));
  ...
  private Authentication generateAuthentication(final List<DataBuffer> dataList, final String encoding) {
    String username = "";
    String password = "";
    String captcha = "";
    ...
    for (final String param : dataParameter) {
      ...
      if (keyValue.length == 2) {
        try {
          ...
          AuthenticationConverter.LOGGER.error("parse kev value exception, key value: {}", keyValue, e);
        }
      }
    }
    return map;
