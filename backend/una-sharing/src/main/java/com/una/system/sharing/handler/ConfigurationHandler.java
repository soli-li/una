...
public class ConfigurationHandler {
  ...
  public Mono<ServerResponse> getAll(final ServerRequest request) {
    ...
    final ScanOptionsBuilder scanOptionsBuilder = ScanOptions.scanOptions()
        .match(this.getConfigurationKey("*")).count(10);
    ...
    final Mono<List<String>> responseMono = scan.buffer(100).collectList().map(o -> {
      ...
      o.forEach(l -> l
          .forEach(id -> keyList.add(StringUtils.substringAfter(id, Constants.REDIS_CONF_PRE))));
      return keyList;
    });
  ...
  public Mono<ServerResponse> getOrEmpty(final ServerRequest request) {
    ...
    ResultMonoUtil.assertTrue(StringUtils.isBlank(companyId), "companyId is empty",
        ConfigurationHandler.LOGGER);

    final TypeReference<List<Configuration>> typeRef = new TypeReference<>() {
    };

    final String redisKey = this.getConfigurationKey(companyId);
    final Mono<String> valueMono = ResultMonoUtil.getExistOrEmptyValue(this.confRedisOperations,
        redisKey, null, ignoreNotExist);
  ...
  public Mono<ServerResponse> remove(final ServerRequest request) {
    ...
    final String companyId = request.pathVariable("companyId");
    final Mono<Long> responseMono = this.confRedisOperations
        .delete(this.getConfigurationKey(companyId));
    return ResultMonoUtil.getSuccessResponse(vo, responseMono);
  }
  ...
  public Mono<ServerResponse> save(final ServerRequest request) {
    ...
    ParameterizedTypeReference<List<Configuration>> typeRef = null;
    typeRef = new ParameterizedTypeReference<>() {
    };
    final Mono<Optional<List<Configuration>>> requestMono = request.bodyToMono(typeRef)
