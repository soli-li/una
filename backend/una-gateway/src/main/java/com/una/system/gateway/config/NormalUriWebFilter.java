...
public class NormalUriWebFilter implements WebFilter {
  ...
  public void init() {
    ...
    NormalUriWebFilter.LOGGER.info(
        "using spring properties key '{}', value is: {}, default value is: {}",
        NormalUriWebFilter.SPRING_REQUEST_OUT_UUID, value, defaultValue);
    NormalUriWebFilter.outUuid = Boolean.parseBoolean(
        environment.getProperty(NormalUriWebFilter.SPRING_REQUEST_OUT_UUID, defaultValue));

    value = environment.getProperty(NormalUriWebFilter.SPRING_USING_HEAD_UUID);
    NormalUriWebFilter.LOGGER.info("using spring properties key '{}', value is: {}",
        NormalUriWebFilter.SPRING_USING_HEAD_UUID, value);
    NormalUriWebFilter.headUuidKey = environment
        .getProperty(NormalUriWebFilter.SPRING_USING_HEAD_UUID);
  }
  ...
  public void markToMdc(final ServerWebExchange exchange) {
    ...
    if (StringUtils.hasText(this.getHeadUuidKey()) && !CollectionUtils.isEmpty(headUuidValue)) {
      ...
      exchange.mutate().request(
          request.mutate().headers(h -> h.set(NormalUriWebFilter.headUuidKey, tempUuid)).build());
    }
  ...
  private void postHandle(final ServerWebExchange exchange) {
    final Object durationFlag = exchange.getAttribute(NormalUriWebFilter.END_TIME);
    if (Objects.isNull(durationFlag)) {
      final LocalDateTime localDateTime = (LocalDateTime) exchange.getAttributes()
          .get(NormalUriWebFilter.START_TIME);
      ...
      NormalUriWebFilter.LOGGER.info("requested done, request uri: {}, now is: {}, used {}ms",
  ...
  private void preHandle(final ServerWebExchange exchange) {
    ...
    if (Objects.isNull(startTimeFlag)) {
      ...
      NormalUriWebFilter.LOGGER.info("request uri: {}, method: {}, now is: {}", request.getPath(),
          request.getMethod(),
