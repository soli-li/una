...
public class ManagerService {
  ...
  public Mono<User> fillUser(final User user) {
    ...
    final WebClient webClient = WebClient.builder().baseUrl(this.managerPath)
        .filter(this.lbFunction).build();
  ...
  public Mono<List<Permissions>> findPermByRole(final Set<String> roleIds) {
    ...
    ParameterizedTypeReference<List<Permissions>> paramTypeRef = null;
    paramTypeRef = new ParameterizedTypeReference<>() {
    };
    return requestBodySpec.retrieve().bodyToMono(paramTypeRef);
  }
  ...
  public Mono<User> findUser(final String name) {
    ...
    final String uuid = MDC.get(Constants.LOG_MARK.toString());
    final String contentType = String.join("",
        Constants.MEDIA_TYPE_X_WWW_FORM_URLENCODED.toString(), ";charset=", this.encoding);
  ...
  public Mono<Boolean> passwordMatch(final String name, final String rawPassword) {
    ...
    requestBodySpec = requestBodySpec.header(this.normalUriWebFilter.getHeadUuidKey(), uuid);
    requestBodySpec
        .body(BodyInserters.fromFormData("username", name).with("password", rawPassword));
